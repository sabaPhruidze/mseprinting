...
import { fetchGiftsAwardsIncentivesData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function GiftsAwardsIncentives() {
  const [giftsAwardsIncentivesData, setGiftsAwardsIncentivesData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getGiftsAwardsIncentivesData = async () => {
      const data = await fetchGiftsAwardsIncentivesData();
      if (data) {
        setGiftsAwardsIncentivesData(data);
      }
    };

    getGiftsAwardsIncentivesData();
  }, []);

  const memoizedData = useMemo(
    () => giftsAwardsIncentivesData,
    [giftsAwardsIncentivesData]
  );
