...
import { fetchPrePostShowDirectMailData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function PrePostShowDirectMail() {
  const [prePostShowDirectMailData, setPrePostShowDirectMailData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getPrePostShowDirectMailData = async () => {
      const data = await fetchPrePostShowDirectMailData();
      if (data) {
        setPrePostShowDirectMailData(data);
      }
    };

    getPrePostShowDirectMailData();
  }, []);

  const memoizedData = useMemo(
    () => prePostShowDirectMailData,
    [prePostShowDirectMailData]
  );
