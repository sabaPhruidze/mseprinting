...
import { fetchCounterPopUpDisplaysData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function CounterPopUpDisplays() {
  const [counterPopUpDisplaysData, setCounterPopUpDisplaysData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getCounterPopUpDisplaysData = async () => {
      const data = await fetchCounterPopUpDisplaysData();
      if (data) {
        setCounterPopUpDisplaysData(data);
      }
    };

    getCounterPopUpDisplaysData();
  }, []);

  const memoizedData = useMemo(
    () => counterPopUpDisplaysData,
    [counterPopUpDisplaysData]
  );
