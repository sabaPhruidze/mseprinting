...
import { fetchEventLiteratureSignsData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function EventLiteratureSigns() {
  const [eventLiteratureSignsData, setEventLiteratureSignsData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getEventLiteratureSignsData = async () => {
      const data = await fetchEventLiteratureSignsData();
      if (data) {
        setEventLiteratureSignsData(data);
      }
    };

    getEventLiteratureSignsData();
  }, []);

  const memoizedData = useMemo(
    () => eventLiteratureSignsData,
    [eventLiteratureSignsData]
  );
