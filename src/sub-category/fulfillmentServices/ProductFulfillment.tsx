...
import { fetchProductFulfillmentData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function ProductFulfillment() {
  const [productFulfillmentData, setProductFulfillmentData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getProductFulfillmentData = async () => {
      const data = await fetchProductFulfillmentData();
      if (data) {
        setProductFulfillmentData(data);
      }
    };

    getProductFulfillmentData();
  }, []);

  const memoizedData = useMemo(
    () => productFulfillmentData,
    [productFulfillmentData]
  );
