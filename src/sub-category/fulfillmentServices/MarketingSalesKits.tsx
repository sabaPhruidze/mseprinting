...
import { fetchMarketingSalesKitsData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function MarketingSalesKits() {
  const [marketingSalesKitsData, setMarketingSalesKitsData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getMarketingSalesKitsData = async () => {
      const data = await fetchMarketingSalesKitsData();
      if (data) {
        setMarketingSalesKitsData(data);
      }
    };

    getMarketingSalesKitsData();
  }, []);

  const memoizedData = useMemo(
    () => marketingSalesKitsData,
    [marketingSalesKitsData]
  );
