...
import { fetchCampaignsConsultationData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function CampaignsConsultation() {
  const [campaignsConsultationData, setCampaignsConsultationData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getCampaignsConsultationData = async () => {
      const data = await fetchCampaignsConsultationData();
      if (data) {
        setCampaignsConsultationData(data);
      }
    };

    getCampaignsConsultationData();
  }, []);

  const memoizedData = useMemo(
    () => campaignsConsultationData,
    [campaignsConsultationData]
  );
