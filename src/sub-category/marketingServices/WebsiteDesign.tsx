...
import { fetchWebsiteDesignData } from "../../data/sub-category data/AllSubCategoryData";
...
export default function WebsiteDesign() {
  const [websiteDesignData, setWebsiteDesignData] =
    useState<SubCategoryCommonTypes | null>(null);

  useEffect(() => {
    const getWebsiteDesignData = async () => {
      const data = await fetchWebsiteDesignData();
      if (data) {
        setWebsiteDesignData(data);
      }
    };

    getWebsiteDesignData();
  }, []);

  const memoizedData = useMemo(() => websiteDesignData, [websiteDesignData]);
